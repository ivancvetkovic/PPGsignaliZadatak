...
# Broj signala za prikaz
num_signals = len(ppg_signals)

# Redosled signala u matrici
rows = int(np.ceil(np.sqrt(num_signals)))
cols = int(np.ceil(num_signals / rows))

# Prikaz svakog signala
...
# Odabir skala za rekonstrukciju
selected_scales = [1, 2, 3]  # Promenite ovo na željene skale

# Rekonstrukcija signala koristeći odabrane skale
reconstructed_signal = pywt.waverec(coeffs[:level+1], wavelet)

# Normalizacija rekonstruisanog signala na opseg [-1, 1]
reconstructed_signal = (reconstructed_signal - np.mean(reconstructed_signal)) / np.max(np.abs(reconstructed_signal))

# Prikazivanje originalnog signala i rekonstruisanog signala
plt.figure()
plt.plot(ppg_data, label='Originalni signal')
plt.plot(reconstructed_signal, label='Rekonstruisani signal')
plt.xlabel('Vremenski odbirak')
plt.ylabel('Amplituda')
plt.title('Wavelet transformacija')
plt.legend()
plt.show()
